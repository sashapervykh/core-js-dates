...
function getTime(date) {
  ...
  return str.match(/[0-9][0-9]:[0-9][0-9]:[0-9][0-9]/);
}
...
function getNextFriday(date) {
  ...
  const res = date.valueOf() + 86400 * 1000 * dayToFriday;
  return new Date(res);
}
...
function getCountDaysInMonth(month, year) {
  ...
  const newDate = new Date(
    `${nextYear.toString().padStart(2, '0')}-${nextMonth
      .toString()
      .padStart(2, '0')}-01`
  );
  ...
  const amountOfDays = (newDate.valueOf() - currDate.valueOf()) / 86400 / 1000;
  return amountOfDays;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const a = new Date(Date.parse(dateStart));
  const b = new Date(Date.parse(dateEnd));
  return (b.valueOf() - a.valueOf()) / 86400 / 1000 + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  if (start <= checkedDate && checkedDate <= end) return true;
  return false;
}
...
function formatDate(date) {
  ...
  const partOfDay = data.getHours() > 11 ? 'PM' : 'AM';
  const minutes = data.getMinutes();
  const seconds = data.getSeconds();
  const month = data.getMonth() + 1;
  const year = data.getFullYear();
  const day = data.getDate();
  return `${month}/${day}/${year}, ${hours}:${minutes.toString().padStart(2, '0')}:${seconds
    .toString()
...
function getCountWeekendsInMonth(month, year) {
  let date = new Date(`${year}-${month.toString().padStart(2, '0')}-01`);

  let counter = date.valueOf();
...
function getWeekNumberByDate(date) {
  ...
  const firstJan = new Date(year, 0, 2);

  const step = [-6, -7, 0, -2, -3, -4, -5];
  const startDate = new Date(
    firstJan.valueOf() + step[firstJan.getDay()] * 86400 * 1000
  );

  const amountOfWeeks =
    Math.floor((currDate.valueOf() - startDate.valueOf()) / 86400 / 1000 / 7) +
    1;
...
function getNextFridayThe13th(date) {
  let counter = date.valueOf() + 86400 * 1000;
  let nextDate = new Date(counter);
  while (nextDate.getDate() !== 13 || nextDate.getDay() !== 5) {
    counter += 86400 * 1000;
    nextDate = new Date(counter);
  }
  return nextDate;
}
