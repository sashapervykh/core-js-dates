...
function getNextFriday(date) {
  ...
  let dateToFriday;
  if (currentDay < 5) {
    dateToFriday = 5 - currentDay;
  } else {
    dateToFriday = 7 - Math.abs(5 - currentDay);
  }
  const fridayDate = date.getTime() + dateToFriday * 24 * 60 * 60 * 1000;
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (date.getMonth() === month - 1) {
    if (date.getDay() === 0 || date.getDay() === 6) amountOfWeekends += 1;
    dateMilliseconds += 24 * 1000 * 60 * 60;
...
function getWeekNumberByDate(date) {
  ...
  const daysFromBeginning = (date - firstDay) / 1000 / 60 / 60 / 24 + 1;
  if (firstWeekDay === 5 || firstWeekDay === 6)
    return Math.ceil(daysFromBeginning / 7);
  return Math.ceil(daysFromBeginning / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  while (currentDate.getDate() !== 13 || currentDate.getDay() !== 5) {
    const milliseconds = currentDate.getTime() + 24 * 1000 * 60 * 60;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    ...
    const date = currentDate.getUTCDate();
    currentDate.setUTCDate(date + countOffDays);
  }
